...
class User extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('username, password, accountType', 'required'),
			array('email', 'length', 'max'=>255),
			array('username, password', 'length', 'max'=>64),
			array('username', 'length', 'min'=>6),
			array('password', 'length', 'min'=>8),
			array('accountType, studentId, staffId', 'length', 'max'=>10),
                        array('username','checkAvailability'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, username, email, password, accountType, studentId, staffId, created, modified', 'safe', 'on'=>'search'),
		);
	}
	...
	public function hashPassword($password)
	{
        ...
        public function beforeSave() {
            ...
            if ($this->isNewRecord) 
                $this->created = NULL;
            if ($this->scenario == 'insert' || $this->scenario == 'updatePass') {
